 ...
 * @param {FormData} formData - FormData with fields: title, slug (optional), cardBody, description, sortOrder, isActive, image
 */
...
export async function toggleCourseAvailability(id) {
  try {
    const response = await adminClient.put(`/admin/courses/${id}/toggle-availability`);
    return response.data?.data || null;
  } catch (error) {
